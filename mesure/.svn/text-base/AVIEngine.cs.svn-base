...
/// a partir de http://www.adp-gmbh.ch/csharp/avi/write_avi.html
/// 

namespace AVISystem
{
    public class AVIEngine
    {
  ...
  /// <summary>
  /// ouvre un avi en entree et en sortie
  /// </summary>
  ...
  public Bitmap Open(string fileName, UInt32 frameRate, int width, int height) {
    ...
    bmp_       = new Bitmap(width,height,PixelFormat.Format32bppArgb);
    BitmapData bmpDat = bmp_.LockBits(new Rectangle(0,0,width, height), ImageLockMode.ReadOnly, PixelFormat.Format32bppRgb);
    stride_    = (UInt32) bmpDat.Stride;
    bmp_.UnlockBits(bmpDat);
    
    AVIFileInit(); // init la librairie vfw

    // cree un avi en ecriture
  ...
  /// <summary>
  /// ecrit la bitmap bmp_ dans la sortie
  /// </summary>
  public void AddFrame(Bitmap bmp) 
  {
    BitmapData bmpDat = bmp.LockBits(new Rectangle(0,0,(int) width_, (int) height_),ImageLockMode.ReadOnly,PixelFormat.Format24bppRgb);
